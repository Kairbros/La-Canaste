  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--background)' }}>
      <header style={{ background: 'var(--primary)', color: '#fff', position: 'sticky', top: 0, zIndex: 30, boxShadow: 'var(--shadow-level-1)' }}>
        <div style={{ maxWidth: 760, margin: '0 auto', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <h1 style={{ fontSize: 20, fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8 }}><Bike size={22} /> Mis entregas</h1>
            <p style={{ fontSize: 12, opacity: 0.85 }}>{user?.name}</p>
          </div>
          <button onClick={handleLogout} style={{ background: 'rgba(255,255,255,0.18)', border: 'none', color: '#fff', borderRadius: 'var(--rounded-full)', padding: '8px 16px', fontWeight: 600, cursor: 'pointer' }}>
            Salir
          </button>
        </div>
      </header>

      <div style={{ maxWidth: 760, margin: '0 auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        {orders.length === 0 && (
    ...
    <div style={{
      ...
      <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
          <div>
            <p style={{ fontWeight: 800, fontSize: 17, color: 'var(--on-surface)' }}>
              {order.clientName} <span style={{ color: 'var(--on-surface-variant)', fontWeight: 500, fontSize: 14 }}>#{order.id}</span>
            </p>
            <p style={{ fontSize: 14, color: 'var(--on-surface-variant)', marginTop: 2, display: 'flex', alignItems: 'center', gap: 5 }}><MapPin size={15} /> {order.address}</p>
          ...
          <span style={{ background: st.bg, color: st.color, fontSize: 12, fontWeight: 700, padding: '5px 12px', borderRadius: 'var(--rounded-full)', whiteSpace: 'nowrap' }}>
            {st.label}
          </span>
        </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            {mapsUrl && (
              <a href={mapsUrl} target="_blank" rel="noreferrer"
                style={{ background: '#1c4ed8', color: '#fff', fontSize: 14, fontWeight: 700, padding: '10px 18px', borderRadius: 'var(--rounded-full)', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                <Navigation size={16} /> Navegar
              </a>
            )}
            {order.status !== 'EN_CAMINO' && order.status !== 'ENTREGADO' && (
              <button onClick={() => onStatus(order.id, 'EN_CAMINO')}
                style={{ background: 'var(--inverse-surface)', color: '#fff', border: 'none', fontSize: 14, fontWeight: 700, padding: '10px 18px', borderRadius: 'var(--rounded-full)', cursor: 'pointer' }}>
                Marcar en camino
              </button>
            )}
            {order.status !== 'ENTREGADO' && (
              <button onClick={() => onStatus(order.id, 'ENTREGADO')}
                style={{ background: 'var(--primary)', color: '#fff', border: 'none', fontSize: 14, fontWeight: 700, padding: '10px 18px', borderRadius: 'var(--rounded-full)', cursor: 'pointer' }}>
                Marcar entregado
              </button>
            )}
          </div>
